...
# ------------------------------------------------------------
    ...
    if filtered_df.empty:
        ...
        st.caption(
            f"{len(filtered_order_ids)} order(s) zichtbaar na filters · "
            f"{len(selected_order_ids)} geselecteerd"
        )

        # Export gebruikt exact dezelfde actieve filters als de tabel.
        ...
        cust_df = export_base[
            [c for c in cust_rows + ["YearWeek", "Quantity"] if c in export_base.columns]
        ].copy()
        sup_df = export_base[
            [c for c in sup_rows + ["YearWeek", "Quantity"] if c in export_base.columns]
        ].copy()
        ...
        cust_file = (
            _excel_export_bytes(cust_pivot, f"GPC Orders {datetime.now().year}")
            if not cust_disabled else None
        )
        sup_file = (
            _excel_export_bytes(sup_pivot, f"GPC Orders {datetime.now().year}")
            if not sup_disabled else None
        )

        st.markdown("#### Acties")
        a1, a2, a3 = st.columns([1.5, 1.0, 1.4])

        with a1:
            delete_mode = st.selectbox(
                "Verwijderen",
                options=["Geselecteerde orders", "Alle gefilterde orders"],
                key="orders_delete_mode"
            )

            if st.button("Verwijderen", use_container_width=True):
                if delete_mode == "Geselecteerde orders":
                    ...
                    if not filtered_order_ids:
                        ...
                        st.session_state["pending_filtered_delete"] = True
                    ...
                    for _, row in edited_orders.iterrows():
                        ...
                        if pd.notna(oid):
                            ...
                            if oid in orders_base.index:
                                ...
                                orders_base.at[oid, "sales_price"] = (
                                    None if pd.isna(sales_price) else round(float(sales_price), 2)
                                )
        ...
        with a3:
            export_type = st.selectbox(
                ...
                key="orders_export_type"
            )

            if export_type == "Customer":
                export_file = cust_file
                export_disabled = cust_disabled
                export_name = f"GPC_Orders_Customer_{datetime.now().year}.xlsx"
            else:
                export_file = sup_file
                export_disabled = sup_disabled
                export_name = f"GPC_Orders_Supplier_{datetime.now().year}.xlsx"

            st.download_button(
                "Excel export",
                data=export_file.getvalue() if export_file else b"",
        ...
        if st.session_state.get("pending_filtered_delete", False):
            st.warning(
                f"Je staat op het punt alle {len(filtered_order_ids)} zichtbaar gefilterde orders te verwijderen."
            )
            c_yes, c_no, _ = st.columns([1.2, 1, 5])
            ...
            with c_yes:
                if st.button(
                    ...
                    type="primary",
                    use_container_width=True,
                    key="confirm_filtered_delete_yes"
                ):
                    st.session_state.orders = st.session_state.orders[
                        ~st.session_state.orders["id"].isin(filtered_order_ids)
                    ]
                    st.session_state["pending_filtered_delete"] = False
                    save_data()
                    st.success(
                        f"Verwijderd: {len(filtered_order_ids)} gefilterde order(s)."
                    )
                    st.rerun()
            ...
            with c_no:
                if st.button(
                    "Annuleren",
                    use_container_width=True,
                    key="confirm_filtered_delete_no"
                ):
                    st.session_state["pending_filtered_delete"] = False
                    st.rerun()
